...
GitHub is built from a copy-only export snapshot. The source worktree is never deleted. The publish allowlist consists of R12 Python package code, `scripts/r12_*.py`, R12 tests, R12 configuration, README, requirements, and R12 documentation. Generated `output/`, private artifacts, Dataset-A data, model weights, caches, `.arena/`, `.superpowers/`, and `.pytest_cache/` are not committed.
